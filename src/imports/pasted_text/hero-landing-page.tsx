Build a full-screen hero landing page for a Product Designer called “Rico Lavender” using React, TypeScript, Vite, and Tailwind CSS. Here is every detail:
...
- **Logo (left):** Flex row with `gap-3`. Text “Rico Lavender(R)”, `tracking-tight`, black, using `var(--font-heading)`. Beside it, a decorative asterisk character `✳︎` at `text-[25px] sm:text-[30px]`, black, `select-none`, `letter-spacing: -0.02em`.
- **Desktop nav links (center, hidden below md):** Flex row, `text-[23px]`, black. Links: "Labs" (navigates to ‘https://thebox.design'), “RicoGPT” (navigates to ‘https://ai.ricolavender.com’), "Shop" (navigates to ‘https://rlmercantile.com') separated by commas rendered as `, `. Each link has `hover:opacity-60 transition-opacity`.
- **Desktop CTA (right, hidden below md):** An anchor "Get in touch" (opens email client ‘mailto:rico@ricolavender.com’) at `text-[23px]`, black, `underline underline-offset-2`, `hover:opacity-60 transition-opacity`.
...
- Font size: `clamp(18px, 4vw, 26px)`, `line-height: 1.3`, `font-weight: 400`, `color: #000`, `filter: blur(2px)`.
- Two lines of text:
  - Line 1: "Hey there, I’m Rico Lavender,”
  - Line 2: “Nice To Meet You“
- Separated by a `
`.

**2. Typewriter text:**
- Text: `”I am a product and multidisciplinary designer who has spent over 15 years building and scaling the things you’ve used and loved.” `
...
- **4 white pill buttons:** Labels: “Read My Case Studies” (navigates to ‘https://portfolio.ricolavender.com'), “Browse My Designs” (navigates to ‘https://design.ricolavender.com'), “Visit My Store” (navigates to ‘https://rlmercantile.com'), “Get To Know Me” (navigates to ‘https://ai.ricolavender.com'). Each is `inline-flex items-center justify-center bg-white text-black border border-black/10 rounded-full text-[13px] sm:text-[15px] px-4 sm:px-5 py-[0.3em] mx-[0.2em] mb-[0.4em] white-space: nowrap`. Hover: `bg-black text-white`, `transition-colors duration-200`.
- **1 outline pill button:** Text “Contact me: rico@ricolavender.com” (email is underlined with `underline-offset-1`), followed by a small 12x12 copy icon (inline SVG of two overlapping rectangles). Styled: `text-white bg-transparent border border-white rounded-full`, same sizing as above, with `gap-2 sm:gap-3` between text and icon. Hover: `bg-white text-black`. On click, copies “rico@ricolavender.com” to clipboard via `navigator.clipboard.writeText()`.
